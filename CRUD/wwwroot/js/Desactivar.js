var miUrlAct = 'Desactivar';
var seletionAct = 0;
var oneTbodyAct = document.querySelector("#desac tbody"), //Cuerpo de la primera tabla
    //twoTbody = document.querySelector("#two tbody"), //Cuerpo de la segunda tabla
    btnact = document.querySelector("#btn-desc"), //Botón que copiará los datos de las filas seleccionadas
    ...
    seleccionarAct = function (event) { //Función a ejecutarse para seleccionarAct una fila
        if (event.target.tagName == "TD") { //Si se pulsó una celda
            ...
            if (fila.dataset.selected < 1 && seletionAct == 0 && seleccionAct.length == 0) {
                fila.style.backgroundColor = "#FF803C"; //Se la pinta de rojo
    ...
    fActiv = function (event) {
        ...
        if (seleccionAct.length) {
            Swal.fire({
                title: 'Confirmacion',
                text: "Desea Realmente Desactivar El Usuario?",
                icon: 'question',
            ...
            Swal.fire("Error", "Seleccione un Registro a Desactivar", "error")
